<template>
	<view>
		...
		<view class="form">
			<view class="username">
				<input placeholder="请输入账户" v-model="phoneNumber" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="password">
...
<script>
	...
	export default {
		...
		methods: {
			...
			doLogin(){
				uni.hideKeyboard();
				 const validAccounts = ['111111', '222222', '333333', '444444', '555555', '666666'];
				...
				if(this.passwd==""){
					...
					uni.showToast({
						title:"请请输入密码",icon:"none"
					})
					return
				}
				var that = this;
				uni.request({
								url: uni.getStorageSync("url") +'/user/login', //仅为示例，并非真实接口地址。
								method: "POST",
								data:{
